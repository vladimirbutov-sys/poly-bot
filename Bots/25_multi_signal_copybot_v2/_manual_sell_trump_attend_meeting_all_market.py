"""Manual sell: 100% of Trump-attend-meeting NO @ best market bid (aggressive)."""
import sys, io
...
TOKEN = "11585986354449505070941372839758364442452109881575832695898602282671084240924"
TITLE = "Will Donald Trump attend the next US x Iran diplomatic meeting?"
...
SHARES = round(onchain - 0.01, 2)  # safety margin
# Undercut bid by 0.005 to ensure crossing (fill at best available bid price)
LIMIT_PRICE = round(max(bid - 0.005, 0.01), 4)

print(f"Position: on-chain {onchain:.4f} sh  tracker {p.get('size_shares')} sh")
