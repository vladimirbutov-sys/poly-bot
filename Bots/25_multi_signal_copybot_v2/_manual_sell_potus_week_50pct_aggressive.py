"""Aggressive market sell: 50% Trump POTUS NO — limit = bid - 2c to cross the book."""
import sys, io
...
data = tracker.load()
position_key, total_shares = None, 0.0
...
# Undercut bid by 2c to ensure crossing (we still receive the true best bid price in the fill)
LIMIT_PRICE = round(max(bid - 0.02, 0.01), 4)
...
print(f"  (will fill at best available bid ≈${bid:.4f})")
