"""Cancel stale $0.94 limit, sell 100% Israel-attack NO at market (cross bid)."""
import sys, io
...
TOKEN = "22461574847104976677124723854389208007201865418102388935875021506968919487748"
TITLE = "Will Israel conduct military action against Iran by April 21, 2026?"
KEY   = "0xmanual_ffcd47785cabe964591515c30877375c"
STALE_ORDER = "0x4091fa91ed5f3e9e8c1313"  # partial id, will match-find
SHARES_TO_SELL = 78.95
LIMIT_PRICE = 0.87  # crosses bid $0.883 → fills at best bid

# Step 1: find and cancel the live $0.94 sell order
c = executor._get_client()
orders = [o for o in c.get_orders() if str(o.get('asset_id','')) == TOKEN]
for o in orders:
    oid_full = o.get('id', '')
    if o.get('side') == 'SELL':
        print(f"Cancelling stale order {oid_full[:24]}... @ ${o.get('price')}")
...
print(f"NO bid/ask: ${bid:.4f}/${ask:.4f}  new limit ${LIMIT_PRICE} (crosses bid)")
