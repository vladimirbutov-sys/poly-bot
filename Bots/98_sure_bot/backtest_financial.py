"""Backtest: how do financial markets at 97.5%+ resolve? Are low-volume ones riskier?"""
import sys
...
while True:
    ...
    if last_end and last_end < "2026-03-01":
        break
    if len(batch) < 500:
        break
    offset += 500
...
# Filter to resolved financial markets
financial_resolved = []
for m in all_markets:
    q = m.get("question", "")
    if not is_financial(q):
        continue
    if is_coinflip(q):
        continue
...
print(f"Resolved financial markets (vol >= $3K): {len(financial_resolved)}")

# Now sample some markets and check their price history via CLOB API
# to see what price was BEFORE resolution
CLOB_HISTORY = "https://clob.polymarket.com/prices-history"

results = {"won": 0, "lost": 0, "profit": 0, "cost": 0}
by_vol = {}
lost_list = []
checked = 0
MAX_CHECK = 200  # limit API calls

print(f"\nChecking price history for up to {MAX_CHECK} markets...")

for fm in financial_resolved[:MAX_CHECK]:
    # For each outcome, check if it had price 0.975+ before resolution
    for i, token_id in enumerate(fm["token_ids"]):
        if i >= len(fm["outcomes"]):
            break
        ...
        # Get last 5 prices before resolution (approximate pre-resolution price)
        prices_list = [pt.get("p", 0) for pt in data[-10:]]
        if not prices_list:
            continue

        # Average of last few prices = approximate entry price
        avg_price = sum(prices_list[-5:]) / len(prices_list[-5:])
        max_price = max(prices_list[-5:])

        # Would bot have bought this? (price in 0.975-0.993)
        if not (0.975 <= avg_price <= 0.993):
            continue
        ...
        cost = 5.0  # $5 bet
        if outcome_won:
            profit = 5.0 / avg_price - 5.0  # shares * $1 - cost
            results["won"] += 1
        else:
            profit = -5.0
            results["lost"] += 1
        ...
        if bucket not in by_vol:
            by_vol[bucket] = {"won": 0, "lost": 0}
        if outcome_won:
            by_vol[bucket]["won"] += 1
        else:
            by_vol[bucket]["lost"] += 1
